...
#         print('====================================')
    ...
    def __train_face_and_pos__(self, fls, aus, embs, face_id, smooth_win=31, close_mouth_ratio=.99):
        ...
        z = torch.zeros(aus.shape[0], 128).clone().requires_grad_(False).to(device)
    ...
    def __train_pass__(self, au_emb=None, centerize_face=False, no_y_rotation=False, vis_fls=False):
        ...
        for i, batch in enumerate(dataloader):
            ...
            for key in keys: # ['45hn7-LXDX8']: #['sxCbrYjBsGA']:#
                ...
                result['pred_fls_{}_{}.pkl'.format(video_name.split('\\')[-1].split('/')[-1], key)] = fake_fls_np
#                 filename = 'pred_fls_{}_{}.pkl'.format(video_name.split('\\')[-1].split('/')[-1], key)
#                 with open(os.path.join(self.opt_parser.output_folder, filename), "wb") as f:
#                     pickle.dump(fake_fls_np, f)
#                 np.savetxt(, fake_fls_np, fmt='%.6f')
    ...
    def test(self, au_emb=None):
        with torch.no_grad():
            return self.__train_pass__(au_emb, vis_fls=False)
