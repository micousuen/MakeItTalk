...
class Image_translation_block():
    ...
    def single_test(self, jpg=None, fls=None, filename=None, prefix='', grey_only=False, chunk_size=50):
        import time
        st = time.time()
        self.G.eval()
...
#         writer = cv2.VideoWriter('out.mp4', cv2.VideoWriter_fourcc(*'mp4v'), 62.5, (256 * 3, 256))
        writer = cv2.VideoWriter('out.mp4', cv2.VideoWriter_fourcc(*'mp4v'), 62.5, (256, 256))
 
        fls_chunks = [fls[i:i+chunk_size] for i in range(0, len(fls), chunk_size)]
        g_out_list = []
        for fls_chunk in fls_chunks:
            images_in = []
            for i, frame in enumerate(fls_chunk):
                ...
                image_in = frame.transpose((2, 0, 1)).reshape(-1, 6, 256, 256)
                images_in.append(image_in)
                
            images_in_stack = np.concatenate(images_in, axis=0)
            images_in_stack = torch.tensor(images_in_stack, requires_grad=False)
            images_in_stack = images_in_stack.to(device)
    
            
            g_out_chunk = self.G(images_in_stack)
            g_out_chunk = torch.tanh(g_out_chunk)
            g_out_chunk = g_out_chunk.cpu().detach()
            g_out_list.append(g_out_chunk)
            
        g_out = np.concatenate(g_out_list, axis=0)
        g_out = g_out.transpose((0, 2, 3, 1))
        g_out[g_out < 0] = 0
